...
if handles.validPlot == true
    try
        [handles.xNew, handles.yNew, handles.yBase] = drawGraph(handles.x, handles.y,...
...
dlmwrite(strcat(PathName, FileName), correctedSpectra,...
        'delimiter', '\t', 'newline', 'pc', 'precision', '%10.6f');
baselineSpectra = [[handles.xNew] [handles.yBase]];
BaseFileName = strcat(FileName, '.base');
dlmwrite(strcat(PathName, BaseFileName), baselineSpectra,...
        'delimiter', '\t', 'newline', 'pc', 'precision', '%10.6f');
